...
function render(){
  ...
  if(root){if(!root.classList.contains('pmm-unified-floating-root'))root.classList.add('pmm-unified-floating-root');root.classList.toggle('is-expanded',state.expanded);root.classList.toggle('is-hidden',!state.visible)}
  if(handle.hidden!==!state.visible)handle.hidden=!state.visible;handle.classList.toggle('is-expanded',state.expanded);if(handle.getAttribute('aria-expanded')!==String(state.expanded))handle.setAttribute('aria-expanded',String(state.expanded));
...
function onStoreChange(state,reason){
  const before=renderedState;renderedState=state;
  if(reason==='drag-frame')return;
  if(before&&['visible','expanded','side','profile'].every(key=>before[key]===state[key])&&['x','y','dock'].every(key=>before.position?.[key]===state.position?.[key]))return;
  schedule();
}
...
#${HANDLE_ID}{translate:none!important;transition-property:border-radius,opacity!important}
...
function onViewportChange(){if(gesture||STORE.getState().keyboardEditing)return;if(resizeFrame)TOP.cancelAnimationFrame(resizeFrame);resizeFrame=TOP.requestAnimationFrame(()=>{resizeFrame=0;if(gesture||STORE.getState().keyboardEditing)return;const portrait=geometry.portrait;refreshViewport();const request=readWidthRequest();if(request.profile!==bannerSizing.profile)setBannerWidth(request.width,request.automatic,request.profile,request.autoContent);if(portrait!==geometry.portrait)invalidateAutoWidth();refreshHeader();measurePanel();queueAutoWidth();STORE.syncProfile();const next=clampPosition(STORE.getState().position||defaultPosition());STORE.commit({position:next,side:resolveSide(next)},'viewport');schedule()})}
...
// Palette events are not typography changes; do not pass their Event object as a force-measure flag.
