...
let managedPanel=null,managedDisplay=null,expandFrame=0;
let root=null,handle=null,gesture=null,renderFrame=0,resizeFrame=0,tapTimer=0,longTimer=0,lastTapAt=0,lastTapPoint=null,suppressMouseUntil=0,panelResizeObserver=null;
...
function clampPosition(position,expanded=STORE.getState().expanded,g=geometry){const s=controlSize(expanded,g),groupH=expanded?Math.max(s.h,g.bannerH):s.h;return{x:Math.round(Math.min(Math.max(4,Number(position?.x)||4),Math.max(4,g.vw-s.w-4))),y:Math.round(Math.min(Math.max(6,Number(position?.y)||6),Math.max(6,g.vh-groupH-6))),dock:['left','right'].includes(position?.dock)?position.dock:'free'}}
...
function panelPoint(position,side,g=geometry){const s=controlSize(true,g),wanted=side==='right'?position.x+s.w+8:position.x-g.bannerW-8;return{x:Math.round(Math.min(Math.max(4,wanted),Math.max(4,g.vw-g.bannerW-4))),y:position.y}}
...
function paint(position,side){const point=panelPoint(position,side);handle.style.setProperty('--pmm-floating-x',position.x+'px');handle.style.setProperty('--pmm-floating-y',position.y+'px');handle.dataset.side=side;if(root){root.dataset.side=side;root.style.setProperty('--pmm-banner-x',point.x+'px');root.style.setProperty('--pmm-banner-y',point.y+'px');root.style.setProperty('--pmm-banner-max-width',Math.max(160,geometry.vw-8)+'px')}}
...
function setExpanded(value,reason='toggle'){
  const panel=root?.querySelector?.(':scope > .panel-wrapper');
  const before=STORE.getState(),expanded=Boolean(value)&&Boolean(panel),old=controlSize(before.expanded),next=controlSize(expanded),position=before.position||defaultPosition();
...
function onDown(event){
  if(event.button!=null&&event.button!==0||gesture)return;const fromHandle=event.currentTarget===handle;if(!fromHandle&&interactiveTarget(event.target))return;
  if(fromHandle)blurActiveEditor();const state=STORE.getState(),position=ensurePosition();gesture={id:event.pointerId,fromHandle,target:event.currentTarget,sx:event.clientX,sy:event.clientY,bx:position.x,by:position.y,expanded:state.expanded,panel:state.expanded?root?.querySelector?.(':scope > .panel-wrapper')||null:null,moved:false,longPressed:false,g:{...geometry}};
...
function updateDragPoint(event){
  ...
  const g=gesture.g,s=controlSize(gesture.expanded,g),height=gesture.expanded?Math.max(s.h,g.bannerH):s.h;
  gesture.dx=Math.min(Math.max(4,gesture.bx+point.clientX-gesture.sx),Math.max(4,g.vw-s.w-4))-gesture.bx;
  gesture.dy=Math.min(Math.max(6,gesture.by+point.clientY-gesture.sy),Math.max(6,g.vh-height-6))-gesture.by;
  return point;
}
...
function settle(position,g=geometry){const state=STORE.getState(),s=controlSize(state.expanded,g),distance=Math.min(72,Math.max(40,g.vw*.08));let next={...position,dock:'free'};if(next.x<=distance)next={...next,x:4,dock:'left'};else if(g.vw-next.x-s.w<=distance)next={...next,x:g.vw-s.w-4,dock:'right'};next=clampPosition(next,state.expanded,g);STORE.commit({position:next,side:resolveSide(next,g),gesture:'idle'},'drag-end')}
...
function onUp(event){
  ...
  if(done.fromHandle){handle?.blur?.();singleTap(event);event.preventDefault();event.stopPropagation()}
  else invokeNativeHeaderTap(done.target,event)
}
function onCancel(){gesture=null;clearLong();cancelPendingTap();clearDragPaint();schedule()}
function invokeNativeHeaderTap(target,event){if(!target?.isConnected)return;const Mouse=target.ownerDocument?.defaultView?.MouseEvent||TOP.MouseEvent;const init={bubbles:true,cancelable:true,view:target.ownerDocument?.defaultView||TOP,button:0,buttons:1,clientX:event.clientX,clientY:event.clientY};target.dispatchEvent(new Mouse('mousedown',init));target.ownerDocument.dispatchEvent(new Mouse('mouseup',{...init,buttons:0}))}
...
function suppressSyntheticClick(event){if(!gesture?.suppressClick&&!event.currentTarget.__pmmSuppressClick)return;event.preventDefault();event.stopImmediatePropagation();event.currentTarget.__pmmSuppressClick=false}
...
#preset-manager-floating-panel .pmm-unified-floating-root .quick-edit-dropdown{flex:1 1 auto!important;min-height:0!important;max-height:calc(100dvh - var(--pmm-banner-y,6px) - 50px)!important}
...
/* Expanded banner occupies at most half the current screen; preserve the material. */
#preset-manager-floating-panel .pmm-unified-floating-root>.panel-wrapper.dropdown-open{height:min(var(--pmm-floating-max-height,560px),50dvh,calc(100dvh - var(--pmm-banner-y,6px) - 6px))!important}

#preset-manager-floating-panel .pmm-unified-floating-root>.panel-wrapper{max-height:min(var(--pmm-floating-max-height,560px),50dvh,calc(100dvh - var(--pmm-banner-y,6px) - 6px))!important}
...
function bindRoot(){const found=findRoot();if(found!==root){root=found;root?.querySelectorAll('.pmm-preset-visible-label').forEach(label=>label.remove());watchPanel();measurePanel();schedule()}const header=root?.querySelector?.(':scope > .panel-wrapper > .panel-header');if(header&&!boundHeaders.has(header)){boundHeaders.add(header);listen(header,'pointerdown',onDown,{capture:true,passive:false});listen(header,'click',suppressSyntheticClick,{capture:true})}const collapse=root?.querySelector?.(':scope > .panel-wrapper .panel-collapse');if(collapse&&!boundCollapses.has(collapse)){boundCollapses.add(collapse);listen(collapse,'click',()=>TOP.queueMicrotask(()=>setExpanded(false,'collapse')))}}
...
function onViewportChange(){if(STORE.getState().keyboardEditing)return;if(resizeFrame)TOP.cancelAnimationFrame(resizeFrame);resizeFrame=TOP.requestAnimationFrame(()=>{resizeFrame=0;refreshViewport();measurePanel();STORE.syncProfile();const next=clampPosition(STORE.getState().position||defaultPosition());STORE.commit({position:next,side:resolveSide(next)},'viewport');schedule()})}
