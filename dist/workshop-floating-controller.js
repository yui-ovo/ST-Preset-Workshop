...
function controlSize(expanded,g=geometry,dock='free'){if(expanded)return{w:g.handleW,h:g.handleH};const size=dock==='free'?g.ball:Math.max(44,g.handleW);return{w:size,h:size}}
...
function clampPosition(position,expanded=STORE.getState().expanded,g=geometry){
  ...
  const x=dock==='left'?0:dock==='right'?Math.max(0,g.vw-s.w):Math.min(Math.max(4,Number(position?.x)||4),Math.max(4,g.vw-s.w-4));
  return{x:Math.round(x),y:Math.round(Math.min(Math.max(6,Number(position?.y)||6),Math.max(6,g.vh-groupH-6))),dock};
}
...
function onCancel(event){if(gesture?.id!=null&&event?.pointerId!=null&&gesture.id!==event.pointerId)return;try{gesture?.target?.releasePointerCapture?.(gesture.id)}catch(_){}gesture=null;clearLong();cancelPendingTap();clearDragPaint();schedule()}
...
/* Docked entry keeps a 44px touch target, sits flush at either edge, and opens immediately. */
html body #${HANDLE_ID}.is-docked{width:max(44px,var(--pmm-floating-handle-width,28px))!important;height:max(44px,var(--pmm-floating-handle-width,28px))!important;border-radius:4px!important}
html body #${HANDLE_ID}.is-docked .pmm-ball-glyph{opacity:0}
html body #${HANDLE_ID}.is-docked .pmm-handle-glyph{opacity:1;transform:none!important;font-size:22px}
html body #${HANDLE_ID}.is-docked[data-dock="left"]{border-left:0;border-top-left-radius:0!important;border-bottom-left-radius:0!important}
html body #${HANDLE_ID}.is-docked[data-dock="right"]{border-right:0;border-top-right-radius:0!important;border-bottom-right-radius:0!important}
...
function bindRoot(){const found=findRoot();if(found!==root){root=found;root?.querySelectorAll('.pmm-preset-visible-label').forEach(label=>label.remove());watchPanel();measurePanel();schedule()}const header=root?.querySelector?.(':scope > .panel-wrapper > .panel-header');if(header&&!boundHeaders.has(header)){boundHeaders.add(header);listen(header,'pointerdown',onDown,{capture:true,passive:false});listen(header,'click',suppressSyntheticClick,{capture:true})}if(header&&!header.querySelector('.pmm-entries-toggle')){const button=header.ownerDocument.createElement('button');button.type='button';button.className='pmm-entries-toggle';button.textContent='条目';button.title='展开或收起当前预设条目';button.setAttribute('aria-label',button.title);listen(button,'click',event=>{event.stopPropagation();root?.__pmmQuickEntries?.toggle?.()});header.appendChild(button);cleanup.push(()=>button.remove())}const collapse=root?.querySelector?.(':scope > .panel-wrapper .panel-collapse');if(collapse&&!boundCollapses.has(collapse)){boundCollapses.add(collapse);listen(collapse,'click',()=>TOP.queueMicrotask(()=>setExpanded(false,'collapse')))}}
