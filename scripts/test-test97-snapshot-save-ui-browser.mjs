...
const server=createServer((req,res)=>{const path=new URL(req.url,'http://localhost').pathname;res.setHeader('Content-Type',path.endsWith('.js')?'text/javascript':'text/html; charset=utf-8');res.end(path==='/preset.js'?moduleSource:path==='/snapshot-name-dialog.js'?helper:path==='/scripts/openai.js'?'export const promptManager={render(){}};':html)});
...
try {
  for(const width of [360,390,1280]) for(const tone of ['dark','light']) {
    ...
    // Apply -> adjust -> overwrite / save-as must work without restoring default.
    ...
    const overwriteButton=page.locator(`[data-pmm-snapshot-action="overwrite"][data-pmm-snapshot-id="${mixed.id}"]`);
    const cancelClick=overwriteButton.click();
    const cancelDialog=await cancelledDialog;
    assert.equal(cancelDialog.message(),'用当前开关覆盖快照“原位独立条目”？');
    await cancelDialog.dismiss();await cancelClick;
    assert.deepEqual(await page.evaluate(()=>JSON.parse(localStorage.getItem('pmm.switch-snapshots.v1'))),beforeOverwrite);
    await row.locator('[data-pmm-snapshot-action="menu"]').click();
    ...
    await page.locator('[data-pmm-snapshot-action="new"]').click();
    await page.locator('.pmm-switch-editor-dialog').waitFor();
    const firstSwitch=page.locator('.pmm-switch-editor-standalone [role="switch"]').first();
    assert.equal(await firstSwitch.getAttribute('aria-checked'),'false','Save-as starts from current adjusted switches');
    await firstSwitch.click();
    await page.locator('[data-pmm-editor-action="save"]').click();
    await page.locator('input#name').fill('正在应用时另存');
    await page.locator('button[type="submit"]').click();
    await page.locator('.pmm-switch-snapshot-dialog').waitFor();
    const afterSave=await page.evaluate(()=>JSON.parse(localStorage.getItem('pmm.switch-snapshots.v1')));
    assert.equal(afterSave.snapshots.find(s=>s.name==='正在应用时另存').states[0].enabled,true);
    assert.deepEqual(afterSave.activeSnapshots,beforeOverwrite.activeSnapshots);
    assert.equal(await page.evaluate(()=>fixture.prompts[0].enabled),false,'Save-as does not apply the new draft');
    await page.screenshot({path:fileURLToPath(new URL('active-save-'+width+'-'+tone+'.png',output))});
