...
function browser(width=360,height=800) {
  ...
  class Element extends EventTarget {
    constructor(tag='div'){
      ...
      this.style={getPropertyValue:key=>values.get(key)||'',getPropertyPriority:key=>priorities.get(key)||'',setProperty(key,value,priority=''){if(key==='transform')owner.transformWrites++;values.set(key,String(value));priorities.set(key,priority);},removeProperty(key){values.delete(key);priorities.delete(key);},get cssText(){return [...values].map(([k,v])=>`${k}:${v}${priorities.get(k)?' !important':''}`).join(';');},set cssText(text){values.clear();priorities.clear();for(const item of text.split(';')){const colon=item.indexOf(':');if(colon<0)continue;const key=item.slice(0,colon).trim(),raw=item.slice(colon+1).trim(),priority=/!important$/.test(raw)?'important':'';if(priorities.get(key)&&!priority)continue;values.set(key,raw.replace(/\s*!important$/,''));priorities.set(key,priority);}}};
