...
try {
  for (const width of [360, 390, 1280]) {
    ...
    assert.equal(await page.locator('[data-draft-book] .pmm-wbs-book-head:visible').count(),2,'Both headings visible together');
    await page.screenshot({path:fileURLToPath(new URL('books-collapsed-'+width+'.png',output))});
    await page.locator('[data-draft-book="剧情补充"] .pmm-wbs-book-head').click();
    assert.equal(await page.locator('[data-draft-book="剧情补充"]').getAttribute('data-open'),'true','Book shell opens in the same click');
    assert.equal(await page.locator('[data-draft-book="剧情补充"] .pmm-wbs-book-head').getAttribute('aria-expanded'),'true','Controlled heading exposes expanded state immediately');
    assert.equal(await page.locator('[data-draft-book="剧情补充"] .pmm-wbs-book-entries').getAttribute('hidden'),null,'Entry shell becomes visible before progressive rows finish');
    ...
    await page.locator('[data-draft-book="剧情补充"] .pmm-wbs-book-head').click();
    await page.locator('[data-draft-book="手动保留"] .pmm-wbs-book-head').click();
